...
export const Header: React.FC<HeaderProps> = ({ onGoHome, onGoDashboard, onGoSettings, onGoAdmin, onLogin, theme, onToggleTheme, userPlan = 'starter' }) => {
  ...
  return (
    <header className="sticky top-0 z-50 glass border-b border-slate-200 dark:border-slate-700 px-6 py-1 flex items-center justify-between">
      ...
      >
        <div className="w-12 h-12 md:w-20 md:h-20 rounded-2xl overflow-hidden transition-transform group-hover:scale-110 bg-white dark:bg-slate-800">
          <img
            src="/logo.png"
            alt="MockMentor Logo"
      ...
      <nav className="flex items-center space-x-4 md:space-x-6" aria-label="Main navigation">
        ...
        {user ? (
          <div className="flex items-center gap-2">
            <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-full ${PLAN_BADGE_STYLES[userPlan]}`}>
              {PLAN_LIMITS[userPlan].label}
            </span>
            {onGoAdmin && (
              <button
                onClick={onGoAdmin}
                className="text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-full bg-red-100 text-red-600 dark:bg-red-900/40 dark:text-red-400 hover:bg-red-200 dark:hover:bg-red-900/60 transition-colors"
          ...
          <button
            onClick={onLogin}
            className="px-5 py-3 bg-slate-900 dark:bg-white dark:text-slate-900 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-800 dark:hover:bg-slate-100 transition-all shadow-xl shadow-slate-200 dark:shadow-none active:scale-95"
          >
            Login
          </button>
        )}
